...
The state.md header no longer embeds a git SHA (SAN-493). Pre-fix the
header included `git-sha: <12-char>`, which was always one-commit-stale
because regen runs pre-commit (per the sealed-gate pattern, HEAD at
regen time is the parent commit's SHA, never the SHA of the commit
landing the state.md update). The SHA is now dropped entirely; commit
SHAs live only in git log.
