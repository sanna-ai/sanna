...
class TestTopLevelExportsMinimal:
    def test_all_has_exactly_16_names(self):
        """sanna.__all__ should contain exactly 16 curated names."""
        import sanna
        assert len(sanna.__all__) == 16

    def test_expected_names_present(self):
        """All 16 curated names should be in __all__."""
        import sanna
        expected = {
            ...
            "ReceiptSink", "SinkResult", "SinkError", "FailurePolicy",
            "LocalSQLiteSink", "NullSink",
        }
        assert set(sanna.__all__) == expected

    def test_imports_actually_work(self):
        """All 16 exports should be importable."""
        from sanna import (
            __version__, sanna_observe, SannaHaltError, SannaResult,
            generate_receipt, SannaReceipt, verify_receipt,
            VerificationResult, ReceiptStore, DriftAnalyzer,
